...
async function actualCallAnswer() {
    ...
    if (!response) {
        fs.writeFile('last_call.txt', date.format(new Date(), 'YYYY/MM/DD HH:mm:ss'), (error) => {
            if(error) throw error;
        ...
        const hours = Math.floor(date.subtract(actualCall, lastCall).toHours());
        const minutes = Math.floor(date.subtract(actualCall, lastCall).toMinutes());
        const seconds = Math.floor(date.subtract(actualCall, lastCall).toSeconds() - date.subtract(actualCall, lastCall).toMinutes() * 60);
        ...
        fs.writeFileSync("last_call.txt", date.format(new Date(), 'YYYY/MM/DD HH:mm:ss'));
